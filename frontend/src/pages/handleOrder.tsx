  ...
  return (
    <main className="flex flex-row gap-10 align-middle">
      <form className="form text-center">
        ...
        {errors.id && (
          <p className="error">
            Please enter the order id you&apos;re resolving.
          </p>
        )}
        <input
          type="file"
          className="input"
          {...register("playerFile", { required: true })}
        />
        {errors.playerFile && (
          <p className="error">
            Please select the excel file containing player data.
          </p>
        )}
        <input
          type="file"
          className="input"
          {...register("leagueFile", { required: true })}
        />
        {errors.leagueFile && (
          <p className="error">
            Please select the excel file containing league data.
          </p>
        )}
        <button
        ...
        {loading && (
          <p className="p-1 text-[14px] font-light text-orange-400">
            Loading...
          </p>
        )}{" "}
  ...
  return (
    <div className="toplevel">
      <Head>
        ...
        <meta
          name="description"
          content="Generate page for Tactalyse employees"
        />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Header />
      <main>{(session?.user.isEmployee && <Access />) || <NoAccess />}</main>
    </div>
  );
}
